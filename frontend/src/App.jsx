...
const [supplierForm, setSupplierForm] = useState({
  ...
  const privateMenuItems = [
    ...
    { label: "Comercial", icon: <Handshake size={18} /> },
...
const simulateRecipeCost = async () => {
  ...
  try {
    const response = await api.get("/lista-valores/tipo-documento");
...
const cargarCiiu = async () => {
  try {
    const response = await api.get("/lista-valores/ciiu");
    setCiiuList(response.data);
  } catch (error) {
    console.error(error);
  }
};
...
const cargarProvincias = async (departamento) => {
  try {
    const response = await api.get(
      `/ubigeo/provincias/${encodeURIComponent(departamento)}`
    );
...
const cargarDistritos = async (departamento, provincia) => {
  try {
    const response = await api.get(
      `/ubigeo/distritos/${encodeURIComponent(
        departamento
  ...
  useEffect(() => {
	...
	useEffect(() => {
	if (activeMenu === "Comercial" && loggedUser) {
		// El módulo Comercial carga sus propios datos internos.
	}
	...
	if (activeMenu === "Inventario" && loggedUser) {
		loadIngredients();
		loadSuppliers();
		loadCatalogs();
		loadPresentations();
...
if (editingSupplier) {
  ...
  console.log("supplierForm antes de guardar:", supplierForm);
  await api.put(`/suppliers/${editingSupplier.proveedor_id}`, payload);
} else {
  ...
  const handleSaveRecipe = async (event) => {
    ...
    try {
      const payload = {
        ...
        labor_cost_type: recipeForm.labor_cost_type || "PERCENTAGE",
        labor_percentage: Number(recipeForm.labor_percentage || 0),
        labor_fixed_amount: Number(recipeForm.labor_fixed_amount || 0),
        estimated_production_minutes: recipeForm.estimated_production_minutes
          ? Number(recipeForm.estimated_production_minutes)
          : null,

        production_overhead_percentage: Number(
          recipeForm.production_overhead_percentage || 0
        ),
        production_overhead_fixed_amount: Number(
          recipeForm.production_overhead_fixed_amount || 0
        ),

        profit_calculation_type: recipeForm.profit_calculation_type || "MARKUP",
        profit_percentage: Number(recipeForm.profit_percentage || 0),
  ...
  try {
    const response = await axios.get(
      `${API_URL}/ubigeo/provincias/${encodeURIComponent(value)}`
    );
...
const handleEditSupplier = (supplier) => {
  ...
  console.log("supplier completo:", supplier);
  setEditingSupplier(supplier);

  setSupplierForm({
    tipo_documento: supplier.tipo_documento || "",
    nro_documento: supplier.nro_documento || "",
    ...
    ciiu: supplier.ciiu || "",
    representante_legal: supplier.representante_legal || "",
  ...
  const handleEditRecipe = (recipe) => {
    ...
    setRecipeForm({
      ...
      labor_cost_type: recipe.labor_cost_type || "PERCENTAGE",
      labor_percentage: recipe.labor_percentage || 0,
      labor_fixed_amount: recipe.labor_fixed_amount || 0,
      estimated_production_minutes: recipe.estimated_production_minutes || "",

      production_overhead_percentage:
        recipe.production_overhead_percentage || 0,
      production_overhead_fixed_amount:
        recipe.production_overhead_fixed_amount || 0,

      profit_calculation_type: recipe.profit_calculation_type || "MARKUP",
      profit_percentage: recipe.profit_percentage || 0,
...
const handleDeleteSupplier = async (supplier) => {
  const confirmDelete = window.confirm(
    `¿Deseas eliminar/desactivar al proveedor "${
  ...
  try {
    await api.delete(`/suppliers/${supplier.proveedor_id}`, {
      data: {
        update_by: loggedUser?.user_id || 1,
      },
    });

    if (editingSupplier?.proveedor_id === supplier.proveedor_id) {
      resetSupplierForm();
    }
...
const filteredSuppliers = suppliers.filter((item) => {
  ...
  return (
    String(item.nro_documento ?? "").toLowerCase().includes(search) ||
    String(item.razon_social ?? "").toLowerCase().includes(search) ||
    String(item.nombre ?? "").toLowerCase().includes(search) ||
    String(item.apellido_paterno ?? "").toLowerCase().includes(search) ||
    String(item.apellido_materno ?? "").toLowerCase().includes(search)
  );
});
...
>
  ...
  const renderSuppliers = () => (
    <div cclassName="max-w-[1800px] mx-auto px-6">
      <div className="mb-8">
        <p className="text-sm font-semibold text-tanta-primary dark:text-[#f0b36d] mb-1">
          Tanta House · Compras
        </p>
...
<div className="space-y-4">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
    ...
    <div>
      <label className="text-sm opacity-80">Tipo Documento</label>
      <select
        value={supplierForm.tipo_documento}
        onChange={(e) =>
          setSupplierForm({
            ...supplierForm,
    ...
    <div>
      ...
      <input
        value={supplierForm.nro_documento}
        onChange={(e) =>
          setSupplierForm({
            ...supplierForm,
  ...
  <div>
    <label className="text-sm opacity-80">Razón Social</label>
    <input
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
    ...
    <div>
      <label className="text-sm opacity-80">Apellido Paterno</label>
      <input
        value={supplierForm.apellido_paterno}
        onChange={(e) =>
          setSupplierForm({
            ...supplierForm,
    ...
    <div>
      <label className="text-sm opacity-80">Apellido Materno</label>
      <input
        value={supplierForm.apellido_materno}
        onChange={(e) =>
          setSupplierForm({
            ...supplierForm,
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
  ...
  <div>
    ...
    <select
      value={supplierForm.departamento}
      onChange={async (e) => {
  ...
  <div>
    <label className="text-sm opacity-80">Provincia</label>
...
<div>
  <label className="text-sm opacity-80">Ciudad / Distrito</label>
...
<div>
  <label className="text-sm opacity-80">CIIU</label>

  <select
    value={supplierForm.ciiu}
    onChange={(e) =>
      setSupplierForm({
        ...supplierForm,
        ciiu: e.target.value,
  ...
  >
	...
	<div>
  <label className="text-sm opacity-80">Calificación</label>
  <input
    value={supplierForm.calificacion}
    onChange={(e) =>
      setSupplierForm({
        ...supplierForm,
...
<div>
  <label className="text-sm opacity-80">Representante Legal</label>
  <input
    value={supplierForm.representante_legal}
    onChange={(e) =>
      setSupplierForm({
        ...supplierForm,
        ...
        <div className="xl:col-span-2 rounded-2xl bg-[#F3EFDC]/85 dark:bg-[#1e1422]/90 border border-tanta-primary/25 dark:border-tanta-primary/35 shadow-card dark:shadow-[0_0_34px_rgba(209,139,73,0.14)] overflow-hidden">
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full min-w-[1120px] table-fixed text-left">
              <thead className="bg-tanta-dark dark:bg-gradient-to-r dark:from-tanta-primary dark:to-[#56599A] text-white">
  <tr>
    <th className="p-4 text-left">Nro. Documento</th>
    <th className="p-4 text-left">Tipo Documento</th>
    <th className="p-4 text-left">Razón Social</th>
    <th className="p-4 text-left">CIIU</th>
...
const calculateVariationPercent = (simulated, current) => {
  ...
  const renderContent = () => {
    ...
    if (activeMenu === "Comercial") return <Sales loggedUser={loggedUser} />;
