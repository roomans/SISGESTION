...
export default function ModalProveedor({
    ...
    useEffect(() => {
        const cargarProveedorEditar = async () => {
            if (!proveedorEditar) {
                return;
            }

            setForm({
                tipo_documento: proveedorEditar.tipo_documento || '06',
                nro_documento: proveedorEditar.nro_documento || '',
                nombre: proveedorEditar.nombre || '',
                apellido_paterno: proveedorEditar.apellido_paterno || '',
                apellido_materno: proveedorEditar.apellido_materno || '',
                razon_social: proveedorEditar.razon_social || '',
                representante_legal: proveedorEditar.representante_legal || '',
                correo: proveedorEditar.correo || '',
                telefono: proveedorEditar.telefono || '',
                departamento: proveedorEditar.departamento || '',
                provincia: proveedorEditar.provincia || '',
                ciudad: proveedorEditar.ciudad || '',
                ubigeo: String(proveedorEditar.ubigeo || ''),
                direccion: proveedorEditar.direccion || '',
                ciiu: proveedorEditar.ciiu || '',
                calificacion: proveedorEditar.calificacion || 'R',
                status: proveedorEditar.status || 'A'
            });

            const provinciasData = await obtenerProvincias(proveedorEditar.departamento);
            setProvincias(provinciasData);

            const ciudadesData = await obtenerDistritos(proveedorEditar.departamento, proveedorEditar.provincia);
    ...
    const cargarInicial = async () => {
        try {
            ...
            const ciiu = await obtenerCatalogo('0002', 'CODIGO_CIIU_SUNAT');
            setCiius(ciiu);
        } catch (error) {
            console.error(error);
        }
    ...
    const guardar = async () => {
        ...
        try {
            ...
            if (proveedorEditar) {
                ...
                // Inyectamos el nro_documento como el nuevo id corporativo propuesto para enlazar la BD
                await crearProveedor({
                    ...form,
                    create_by: usuario.usuario_id,
                    usuario_id: usuario.usuario_id,
                    nuevo_proveedor_id: form.nro_documento 
                });
            }
            ...
            alert(proveedorEditar ? 'Proveedor actualizado correctamente' : 'Proveedor registrado correctamente');
            onSuccess();
            onClose();
        } catch (error) {
    ...
    return (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <div className="card" style={{ width: '800px', maxHeight: '90vh', overflowY: 'auto', padding: '30px' }}>
                ...
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: '600' }}>Correo</label>
                <input
                    style={{ width: '100%', padding: '10px', border: '1px solid #D1D5DB', borderRadius: '6px', marginBottom: '15px' }}
                    value={form.correo}
                ...
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: '600' }}>CIIU</label>
                <select
                    value={form.ciiu || ''}
                ...
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: '600' }}>Distrito</label>
                <select
                    style={{ width: '100%', padding: '10px', border: '1px solid #D1D5DB', borderRadius: '6px', marginBottom: '15px' }}
                    value={form.ubigeo || ''}
                    onChange={(e) => {
                ...
                <textarea
                    ...
                    rows={3}
                    style={{ width: '100%', padding: '10px', border: '1px solid #D1D5DB', borderRadius: '6px', resize: 'none', marginBottom: '15px' }}
                />
